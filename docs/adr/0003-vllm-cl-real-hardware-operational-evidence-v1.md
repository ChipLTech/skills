# vLLM-CL Real Hardware Operational Evidence
...
`vllm-cl-run-spec/v1` remains unchanged. Closed-world
`vllm-cl-run-spec/v2` and `vllm-cl-result-evidence/v2` are introduced for the
...
`vllm-cl-main-to-main-operational-policy/v1` document. This closed-world policy
is approved only for `operational_only` claims and binds the exact vLLM SHA,
vllm-cl SHA, Patch Import Manifest byte digest, DeepSeek operational role at
