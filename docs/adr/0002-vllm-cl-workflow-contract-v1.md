# vLLM-CL workflow contract v1

Ticket 01 needs a deterministic contract in the skills repository while `/work/vllm-cl` remains read-only. This ADR fixes the implementation-level decisions left open by the product spec.

## Contract documents

The three document kinds are `run_spec`, `result_evidence`, and `parent_child_handoff`. Their schema versions are respectively `vllm-cl-run-spec/v1`, `vllm-cl-result-evidence/v1`, and `vllm-cl-parent-child-handoff/v1`.
...
- `target`: `vllm_sha`, `vllm_cl_sha`, and `manifest_digest`.
...
- A non-empty, duplicate-free list of gate identities and an absolute artifact destination outside the read-only vllm-cl root.
...
Skill and knowledge documents must contain the marker `shared_contract: vllm-cl-contract/v1`. They may name gate identities, evidence levels, and reasons. They must not contain:
...
The CLI bootstraps `--vllm-cl-root` before full argument parsing. If that root resolves to a readable Git worktree, every target, including malformed fixtures and later argument failures, reports before and after snapshots. If the root is absent, malformed, or not a readable Git worktree, exit `10` reports `repository_state: not_verified`; it never claims preservation. Snapshot comparison includes HEAD, branch, porcelain status, staged and unstaged binary diff digests, and untracked-content digest.

The CLI never mutates the guarded repository. A synthetic expected-snapshot mismatch is the negative boundary fixture because deliberately mutating `/work/vllm-cl` would violate the product boundary.
