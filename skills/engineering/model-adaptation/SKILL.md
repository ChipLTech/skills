...
5. Prepare the real-weight branch and delegate API and lifecycle behavior exclusively to the shared runner contract. Complete when: the sealed run spec and result identities agree, every runner-owned gate has a terminal state, and unexecuted Real DLC Hardware, Chunked Prefill, or DLC Runtime evidence remains not verified.
