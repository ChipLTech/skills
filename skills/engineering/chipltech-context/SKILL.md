...
4. For capability discovery, task routing, or an unclear owner, read `<KNOWLEDGE_BASE_ROOT>/agent-context/capability-manifest.yaml` as the stable Capability ID closure index, then read `<KNOWLEDGE_BASE_ROOT>/prompt-examples/all-supported-capabilities-quickstart.md` as the canonical human catalog of supported task entrypoints. Select its linked detailed Prompt, Contract, or Runbook; never execute from either catalog alone or treat catalog coverage as execution Evidence.
...
10. Return the Ask Output Contract with literal labels `Selected capability:`, `Selection basis:`, `Minimum missing inputs:`, `First safe action:`, `Expected terminal states:`, and `Evidence boundary:`. Bind `Selected capability:` to the stable Capability ID and owning Skill, include the selected knowledge paths, business contract, execution locus and mount assumptions, and authorization needs, and finish with a literal `Claim Boundary:` stating what is and is not established. For knowledge-only questions, return the cited answer without invoking an execution Skill.

Organization contract timestamps use strict UTC RFC 3339 values ending in literal `Z`. Every organization artifact's `claim_boundary` uses `Claim Boundary: Establishes ...; does not establish ...`. The optional O06 task-plan-round ledger seam applies to a real workflow scenario that is multi-candidate, multi-round, and resumable; it does not require multiple owner adapters and does not introduce a ledger schema until a concrete consumer requires one.
