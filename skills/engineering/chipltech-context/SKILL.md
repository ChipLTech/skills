...
3. Classify the request by problem domain. Search repository-relative filenames, headings, and content before reading broadly. Prefer the relevant topic document, then the nearest applicable case study, then an approved contract under `prompt-examples/`.
4. For every substantive domain claim, cite a repository-relative path plus heading or line range. Label the basis as one of: `direct repository evidence`, `runtime observation`, `inference`, or `missing evidence`. Never upgrade one class into another.
5. Check whether candidate sources conflict, are explicitly historical, or make different claims for different versions. If authority cannot be resolved from repository rules and current identities, stop as `blocked_conflicting_authority` and name the conflicting sources.
6. Route execution to the narrowest approved Skill. Before naming its blocker or Claim Boundary, actually load it through the active Harness and read its scope, negative scope, Inputs, completion criteria, Stop Semantics, and Claim Boundary. Never infer an owner Skill's current contract from this routing table alone or copy its instructions into the answer.
7. Keep the execution workspace unchanged. The knowledge base supplies rationale and contracts; the current business/code repository and task-owned artifact directory supply execution and Evidence.
8. Return the selected knowledge paths, business contract, execution Skill, first executable action, authorization needs, expected terminal states, and Evidence boundary. For knowledge-only questions, return the cited answer without invoking an execution Skill.
