...
8. Report freeze and finalize eligibility without changing metadata. Always identify the Tested Revision that produced evidence. When publication is proposed, separately report the Publication Candidate built on the current target main: full SHA/base, worktree state, scoped diff/tree identity, Patch Equivalence evidence, affected gates rerun after the candidate base, current remote base/PR tip, exact lease expectation, and publication authorization. A stable patch-id or one-commit shape is supporting evidence only and cannot transfer runtime acceptance across a changed base. Complete when: the Tested Revision is unique; any proposed Publication Candidate is unique, fresh for its current base, and still matches the reported remote lease; commit/publication authorization is explicit for each requested action; alignment remains unchanged, manifest remains report-only, and finalize action remains none.
...
- A non-unique Tested Revision, or a proposed Publication Candidate with non-unique identity, incomplete scoped equivalence, stale evidence after a base change, remote lease mismatch, or absent commit/publication authorization, stops before finalize. This candidate never creates worktrees, rewrites history, commits, pushes, or finalizes.
