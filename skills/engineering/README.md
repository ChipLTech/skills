...
- **[model-adaptation](./model-adaptation/SKILL.md)** — Adapt a specific new or incompatible model for loading or serving on the DLC Platform, including versioned topology/payload collective qualification; use for model-level Attention, MLA, MoE, quantization, multimodal, MTP, or distributed compatibility, not upstream alignment, environment rebuilds, single-operator debugging, independent compile work, or running an existing smoke alone.
...
- **[pd-separation](./pd-separation/SKILL.md)** — Deploy or diagnose vLLM-CL Prefill/Decode separation with MooncakeDLCConnector; use for single-node TCP, qualified lyp_full or dlccl_direct, cross-machine TCP, transport qualification, KV-transfer lifecycle, routing, and recovery.
