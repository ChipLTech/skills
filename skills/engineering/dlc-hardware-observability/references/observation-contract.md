...
Preserve raw `cltech_smi` output beside normalized `vllm-cl-smi-observation/v1` JSON. The skills-owned adapter queries device inventory, exclusions, total HBM, and the vendor process table, then cross-checks two Host handle samples and process-group membership. Adapter exit failure is `blocked_missing_observability`, not evidence that a device failed.

## Installation Modes

- Installed skill: use `<SKILL_ROOT>/scripts/qualify-vllm-cl-smi-environment.py` and `<SKILL_ROOT>/scripts/observe-cltech-smi.py`.
