...
- Authorization for each requested pull, network/install, build, device, privileged Host integration profile, export, Host-maintenance, and push action. Device execution never authorizes `--privileged`, Host PID namespace, or Host system mounts.
...
4. Qualify an immutable ordinary daily base and create a task-owned persistent validation environment. Before any model load, close the driver API, device mapping, explicitly authorized C1b-compatible container profile, clean source/submodule worktrees, binary overlay provenance, the CMake toolchain used by actual build subprocesses, and the query-only SMI Observation Envelope owned by `dlc-hardware-observability`. Delegate exact stack identity gating to `dlc-env-setup`: equal Driver/Runtime API plus an approved exact CRT/DLC Custom Kernel/LLVM profile is required before C1b; revoked or unknown profiles are ineligible and static approval is not execution evidence. Reuse a validated profile for the same driver/runtime/base fingerprint on the first attempt; never retry a known minimal-profile failure. Model-specialized, golden, candidate, or unexplained images are ineligible. Follow [Runtime Qualification](references/runtime-qualification.md). **Complete when:** base/container/source/package/device/observer identities and the cleanup baseline are closed.
...
6. Execute real-weight functional assertions, then the declared benchmark. Read the installed server `--help`, use an explicit absolute model path, and set offline Hub guards before starting a server. Capture SMI observations at `before_launch`, `after_ready`, `during_request`, and `after_cleanup` for each server profile; a new benchmark profile is a new epoch. Functional failure blocks benchmark and delivery. Delegate hotspot diagnosis to `diagnosing-bugs`; only the declared formal benchmark epoch can satisfy this workflow's benchmark gate. Distinguish `benchmark_workload_pass` from `benchmark_stability_baseline_pass`; a declared single-run workload may qualify delivery without becoming a stable baseline. **Complete when:** raw requests/results, semantic assertions, `finish_reason`, health-after, server epochs, profile diff, warm-up/formal attempts, raw and normalized SMI evidence, and cleanup evidence are retained.
7. For image delivery, seal runtime evidence before building. Build DLC as a fixed-tag, weight-free image and record exact-image validation separately from pre-build runtime qualification. A changed PyTorch wheel, imported native extension, DLC Custom Kernel binary, KernelDesc adapter, or DLCSynapse/DLC Runtime/DLCCL library invalidates prior execution evidence and requires exact-artifact revalidation. A new Image ID must either run the exact-image gates or carry an exhaustive equivalence record proving every execution-relevant binary and adapter digest unchanged; tag or source equality alone cannot transfer evidence. **Complete when:** image/base/source/build identities, equivalence or changed-binary record, exact-image C1a, applicable C1b/function/benchmark states, tar path/size/SHA-256, attestation, validation level, and target cleanup are closed.
8. Derive TYD from the delivered DLC Image ID through [DLC And TYD Delivery](references/tyd-delivery.md). Driver API changes and any rebuilt native dependency require downstream rebuild through PyTorch, vLLM-CL, and vLLM; never infer a rebuild from an existing base library. **Complete when:** TYD has its own fixed tag, Image ID, tar/hash, provenance, static/exact-image result, prohibited device scope, and final target status.
9. Emit independent DLC/TYD final states and clean only task-owned resources. **Complete when:** task processes/ports/HBM ownership and the `after_cleanup` observation return to the sealed baseline or tolerance; retained deliverables and failed epochs are explicit; no unrelated resource was modified.
...
- Pre-build runtime evidence is not exact-image runtime evidence. Equivalent-environment delivery is allowed only when every execution-relevant binary and adapter digest is unchanged and all remaining differences are bound; otherwise exact-artifact gates remain `not_verified`.
