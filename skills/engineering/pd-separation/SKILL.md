...
1. Freeze the deployment contract before launching. Record full source/package identities, model asset identity, both role profiles, connector/module identity, transport, endpoint and port matrix, request parameters, expected evidence, cleanup baseline, pre-existing workload recovery commands, and explicit exceptions to role symmetry. Read installed `pd_launcher`, proxy, and vLLM `--help`; inspect actual routes instead of assuming `/health`; source examples never override the checked-out implementation. **Complete when:** every identity, endpoint, capability-derived readiness probe, and recovery target is fixed, or a structured blocker names the exact missing input.
...
6. Run one deterministic request through Proxy before concurrency work. Correlate client, Proxy, Prefill, and Decode identities; retain lazy transport initialization when applicable, Prefill block ownership/registration, Decode `remote_request_id`, local/remote block mapping, transport completion, successful device-side receive, response, role health-after, and device/process observations. **Complete when:** request-correlated evidence proves routing and KV consumption, every deferred transport initialization completes, output meets the comparison contract, and both role epochs remain healthy.
