#!/usr/bin/env python3
"""Qualify pinned vLLM-CL launch dependencies without loading a model."""
...
DEFAULT_REQUIREMENTS = Path("/work/skills/requirements-vllm-cl-contracts.txt")
IMPORTS = (
...
NATIVE_MODULES = (
    ("vllm._C", "vllm"),
    ("vllm_cl.vllm_cl_C", "vllm_cl"),
)
SCHEMA_VERSION = "vllm-cl-dependency-qualification/v1"
POLICY_SCHEMA_VERSION = "vllm-cl-dependency-exception-policy/v1"
METADATA_DISTRIBUTIONS = ("vllm", "vllm-cl")
EXPECTED_EXCEPTION = {
...
def qualify_metadata(
    ...
    for check in checks:
        ...
        matches_exception = (
            ...
            and exception["vllm_cl_requirement"]
            in active_requirements.get("vllm-cl", set())
            and any(
                candidate["distribution"] == "vllm-cl"
                and candidate["requirement"] == exception["vllm_cl_requirement"]
...
def qualify_native_modules(
    ...
    expected_vllm_cl_root: Path,
) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    roots = {
        "vllm": expected_vllm_root.resolve(),
        "vllm_cl": expected_vllm_cl_root.resolve(),
    }
    modules = []
    reasons = []
...
def main() -> int:
    ...
    parser.add_argument("--expected-vllm-cl-root", type=Path, default=Path("/work/vllm-cl"))
    ...
    native_modules, native_reasons = qualify_native_modules(
        ...
        arguments.expected_vllm_cl_root,
    )
