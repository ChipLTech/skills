#!/usr/bin/env python3
"""Validate versioned vLLM-CL workflow contracts through one read-only CLI."""
...
CONTRACT_VERSION = "vllm-cl-contract/v1"
DEPENDENCY_QUALIFIER_PATH = Path(__file__).with_name(
    "qualify-vllm-cl-dependencies.py"
).resolve()
DEPENDENCY_REQUIREMENTS_PATH = Path(
    "/work/skills/requirements-vllm-cl-contracts.txt"
).resolve()
DEPENDENCY_EXCEPTION_POLICY_PATH = Path(
    "/work/skills/config/vllm-cl/ticket06-dependency-exceptions.json"
).resolve()
MAIN_TO_MAIN_OPERATIONAL_POLICY_PATH = Path(
    "/work/skills/config/vllm-cl/ticket06-main-to-main-operational-policy.json"
).resolve()
DEPENDENCY_EXCEPTION = {
    "dependency": "opencv-python-headless",
    "distribution": "vllm",
    "installed_version": "4.11.0.86",
    "rationale": "vllm-cl requires <=4.11.0.86 to preserve numpy<2",
    "requirement": "opencv-python-headless>=4.13.0",
    "vllm_cl_requirement": "opencv-python-headless<=4.11.0.86",
}
OPERATIONAL_CAMPAIGN_REQUIRED_IDS = {
    ...
    "vllm_source", "vllm_cl_source", "smi_source",
    "vllm_snapshot", "vllm_cl_snapshot",
    "vllm_native", "vllm_cl_native",
...
def validate_main_to_main_operational_policy(document: Any) -> bool:
    ...
    target_fields = {"manifest_digest", "vllm_cl_sha", "vllm_sha"}
    roles = [
    ...
    if (
        ...
        != "vllm-cl-main-to-main-operational-policy/v1"
    ...
    return (
        ...
        and isinstance(target["vllm_cl_sha"], str)
        and re.fullmatch(SHA_PATTERN, target["vllm_cl_sha"]) is not None
...
def validate_dependency_exception_policy(path: Path) -> bool:
    ...
    return document == {
        ...
        "schema_version": "vllm-cl-dependency-exception-policy/v1",
    }
...
def validate_dependency_qualification(
    ...
    if not isinstance(document, dict) or set(document) != {
        ...
        return False
    if (
        document["schema_version"] != "vllm-cl-dependency-qualification/v1"
    ...
    if (
        ...
        or any(
            ...
            or check["distribution"] not in {"vllm", "vllm-cl"}
        ...
        or {check["distribution"] for check in metadata_checks} != {"vllm", "vllm-cl"}
        ...
        or not any(
            check == {
                ...
                "distribution": "vllm-cl",
                "installed_version": DEPENDENCY_EXCEPTION["installed_version"],
                "requirement": DEPENDENCY_EXCEPTION["vllm_cl_requirement"],
                "satisfied": True,
            }
            for check in metadata_checks
        )
...
SCHEMAS = {
    "run_spec": ("vllm-cl-run-spec/v1", RUN_SPEC_FIELDS),
    "result_evidence": (
        "vllm-cl-result-evidence/v1",
        RESULT_EVIDENCE_FIELDS,
    ),
    "parent_child_handoff": (
        "vllm-cl-parent-child-handoff/v1",
        HANDOFF_FIELDS,
    ),
}
...
MODEL_ADAPTATION_SCHEMA = "vllm-cl-model-adaptation-bundle/v1"
...
MAIN_TO_MAIN_SCHEMA = "vllm-cl-main-to-main-bundle/v1"
MAIN_TO_MAIN_FIELDS = {
    "assignments",
    "baseline",
    "candidate_vllm_cl_sha",
...
KNOWLEDGE_PACKAGE_SCHEMA = "vllm-cl-knowledge-package/v1"
PROMPT_DRY_RUN_SCHEMA = "vllm-cl-prompt-dry-run/v1"
REUSABLE_PROMPT_SCHEMA = "vllm-cl-reusable-prompt/v1"
...
PROMPT_PROFILES = {
    "model_adaptation_prompt": {
        ...
        "required_inputs": (
            ...
            "target_vllm_full_sha", "candidate_vllm_cl_full_sha", "deployment_profile",
...
def bootstrap_guard_root(arguments: list[str]) -> Path | None:
    ...
    for index, argument in enumerate(arguments):
        if argument == "--vllm-cl-root" and index + 1 < len(arguments):
            root = Path(arguments[index + 1])
        if argument.startswith("--vllm-cl-root="):
...
NESTED_FIELDS = {
    "run_spec": {
        "target": {"vllm_sha", "vllm_cl_sha", "manifest_digest"},
        "deployment_profile": {
            "model_id", "model_revision", "tokenizer_revision", "processor_revision",
...
def validate_model_adaptation_derivation(
    ...
    if (
        approval["schema_version"] != "vllm-cl-model-adaptation-approval/v1"
    ...
    if derivation["schema_version"] != "vllm-cl-tp-derivation/v1":
...
def validate_run_spec_v2(
    ...
    exact_objects = (
        ...
        ("target", {"manifest_digest", "vllm_cl_sha", "vllm_sha"}),
    ...
    target = document["target"]
    for field in ("vllm_sha", "vllm_cl_sha"):
    ...
    if document["mode"] == "operational_regression":
        launcher_adapter = Path(__file__).with_name("launch-vllm-cl-server.py").resolve()
        ...
        expected_arguments = [
            ...
            "--expected-vllm-cl-root", document["repository_guards"][1],
        ]
        if (
            ready_file is None
            or port is None
    ...
    if hardware["provider_class"] == "local_process":
        ...
        qualification_adapter = Path(__file__).with_name(
            "qualify-vllm-cl-smi-environment.py"
        ).resolve()
        if (
    ...
    if (
        ...
        or guards != ["/work/vllm", "/work/vllm-cl"]
    ):
...
def validate_operational_result_reference(
    ...
    if reference["schema_version"] != "vllm-cl-result-reference/v2":
    ...
    if (
        result["schema_version"] != "vllm-cl-result-evidence/v2"
    ...
    if run_spec["mode"] == "operational_regression":
        ...
        if (
            ...
            or campaign["schema_version"] != "vllm-cl-execution-campaign/v1"
        ...
        expected_paths = {
            ...
            "runner": Path(__file__).with_name("run-vllm-cl-smoke.py").resolve(),
            "validator": Path(__file__).resolve(),
            "launcher": Path(run_spec["launch"]["arguments"][0]).resolve(),
            "tokenizer_builder": Path(__file__).with_name("build-vllm-cl-long-prefix.py").resolve(),
            ...
            "vllm_cl_source": Path(run_spec["repository_guards"][1]).resolve(),
            ...
            "vllm_cl_native": Path(importlib.util.find_spec("vllm_cl.vllm_cl_C").origin).resolve(),
        ...
        for entry_id, root in (
            ...
            ("vllm_cl_snapshot", Path(run_spec["repository_guards"][1])),
        ):
            try:
    ...
    expected_heads = {
        ...
        run_spec["repository_guards"][1]: run_spec["target"]["vllm_cl_sha"],
    }
    if any(
    ...
    if not isinstance(tokenizer_proof, dict) or set(tokenizer_proof) != {
        ...
        tokenizer_proof["schema_version"] != "vllm-cl-long-prefix-proof/v1"
    ...
    for index, observation in enumerate(observations):
        if set(observation) != {"adapter_schema", "devices", "sample_point"} or observation["adapter_schema"] != "vllm-cl-smi-observation/v1":
...
def validate_ticket06_evidence(
    ...
    if (
        document["schema_version"] != "vllm-cl-ticket06-operational-index/v1"
        or document["claim_level"] != "operational_only"
        or document["alignment_action"] != "unchanged"
        or document["manifest_action"] != "report_only"
        or document["finalization_intent"] != "none"
...
def validate_model_adaptation_operational(
    ...
    if (
        document["schema_version"] != "vllm-cl-model-adaptation-operational/v1"
...
def validate_main_to_main_operational(
    ...
    if (
        document["schema_version"] != "vllm-cl-main-to-main-operational/v1"
...
def nested_shape_error(document: dict[str, Any]) -> dict[str, str] | None:
    for field, expected in NESTED_FIELDS.get(document.get("contract_kind"), {}).items():
        ...
        if missing:
            if field == "target" and missing[0] in {"vllm_sha", "vllm_cl_sha", "manifest_digest"}:
                return {
...
def semantic_error(document: dict[str, Any]) -> dict[str, str] | None:
    ...
    if kind == "run_spec":
        ...
        for field in ("target_vllm_sha", "candidate_vllm_cl_sha"):
...
def package_content_error(
    ...
    ownership = re.compile(
        r"(?m)^\s*shared_contract:\s*vllm-cl-contract/v1\s*$"
    )
...
def validate_knowledge_package(
    ...
    expected_paths = {
        ...
        "decision_record": "vllm-cl/model-adaptation-and-main-to-main-decisions.md",
        "model_adaptation_prompt": "prompt-examples/vllm-cl-model-adaptation.md",
        "main_to_main_prompt": "prompt-examples/vllm-cl-main-to-main-upgrade.md",
    }
    ...
    ownership = re.compile(r"(?m)^\s*shared_contract:\s*vllm-cl-contract/v1\s*$")
    for role in ("decision_record", *sorted(PROMPT_ROLES)):
...
def validate_prompt_dry_run(
    ...
    for index, row in enumerate(cases):
        ...
        for key, value in provided.items():
            ...
            if key in {"target_vllm_full_sha", "candidate_vllm_cl_full_sha"}:
...
def validate_model_adaptation_routing(
    ...
    if (
        ...
        or document["schema_version"] != "vllm-cl-model-adaptation-routing/v1"
...
def validate_main_to_main_routing(
    ...
    if (
        not isinstance(document, dict)
        or set(document) != {"schema_version", "candidate_package", "cases"}
        or document["schema_version"] != "vllm-cl-main-to-main-routing/v1"
        or not isinstance(document["cases"], list)
    ):
        return {"code": "routing.invalid", "status": "failed"}
    candidate = (fixture.parent / document["candidate_package"]).resolve()
...
def validate_model_adaptation_bundle(bundle: dict[str, Any], guarded_root: Path) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    if run_spec is not None:
        ...
        if run_spec["target"]["vllm_cl_sha"] != guarded_snapshot["head"]:
            return bundle_error("contract.identity_mismatch", "$.run_spec.target.vllm_cl_sha"), {}
        profile = run_spec["deployment_profile"]
        expected_identity = {
    ...
    if handoff is not None:
        ...
        expected_handoff = {
            ...
            "candidate_vllm_cl_sha": run_spec["target"]["vllm_cl_sha"],
...
def validate_main_to_main_bundle(
    ...
    if not isinstance(bundle["candidate_vllm_cl_sha"], str) or not re.fullmatch(
        SHA_PATTERN, bundle["candidate_vllm_cl_sha"]
    ):
        return bundle_error("contract.missing_identity", "$.candidate_vllm_cl_sha"), {}
    if bundle["candidate_vllm_cl_sha"] != repository_snapshot(guarded_root)["head"]:
        return bundle_error("contract.identity_mismatch", "$.candidate_vllm_cl_sha"), {}
    ...
    for index, child_row in enumerate(child_rows):
        ...
        expected_values = {
            ...
            "candidate_vllm_cl_sha": (
                handoff["candidate_vllm_cl_sha"],
                bundle["candidate_vllm_cl_sha"],
            ),
    ...
    if (
        ...
        != "vllm-cl-main-to-main-regression-policy/v1"
...
def validate_live_package(skills_root: Path, knowledge_root: Path, identity: str) -> dict[str, str]:
    ...
    required_files = [
        ...
        knowledge_root / "vllm-cl" / "model-adaptation-and-main-to-main-decisions.md",
    ]
...
def main() -> int:
    ...
    parser.add_argument("--vllm-cl-root", required=True, type=Path)
    ...
    try:
        before = (
            bootstrapped_before
            if bootstrapped_root == arguments.vllm_cl_root
            and bootstrapped_before is not None
            else repository_snapshot(arguments.vllm_cl_root)
        )
        if arguments.target == "live-package":
            ...
            contract_check, outcome = validate_model_adaptation_bundle(
                document, arguments.vllm_cl_root
            )
            ...
            document = json.loads(arguments.fixture.read_text(encoding="utf-8"))
            contract_check = validate_contract(document, arguments.vllm_cl_root)
        ...
        return invalid_input_report(error, arguments.vllm_cl_root, before)
    valid = contract_check["status"] == "passed"
    after = repository_snapshot(arguments.vllm_cl_root)
...
def validate_contract(
    document: dict[str, Any], guarded_root: Path
        ...
        if (
            ...
            and document.get("schema_version") == "vllm-cl-run-spec/v2"
        ):
