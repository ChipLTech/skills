#!/usr/bin/env python3
"""Run the shared vLLM-CL HTTP smoke seam and seal simulated evidence."""
...
VALIDATOR_PATH = Path(__file__).with_name("validate-vllm-cl-contracts.py")
TOKENIZER_BUILDER_PATH = Path(__file__).with_name("build-vllm-cl-long-prefix.py")
VALIDATOR_SPEC = importlib.util.spec_from_file_location("vllm_cl_contract", VALIDATOR_PATH)
if VALIDATOR_SPEC is None or VALIDATOR_SPEC.loader is None:
    raise RuntimeError("cannot load the shared vLLM-CL contract")
CONTRACT = importlib.util.module_from_spec(VALIDATOR_SPEC)
VALIDATOR_SPEC.loader.exec_module(CONTRACT)

RESULT_SCHEMA = "vllm-cl-result-evidence/v1"
REFERENCE_SCHEMA = "vllm-cl-result-reference/v1"
API_GATES = (
    "service_ready",
    "models_api",
    "completions_api",
...
V2_RESULT_SCHEMA = "vllm-cl-result-evidence/v2"
...
def run_v2_smi(
    ...
    if (
        observation["adapter_schema"] != "vllm-cl-smi-observation/v1"
...
def validate_campaign_manifest(path: Path, run_spec: dict[str, Any]) -> bool:
    ...
    } or document["schema_version"] != "vllm-cl-execution-campaign/v1":
        return False
    ...
    expected_repositories = {
        ...
        "vllm_cl_source": run_spec["target"]["vllm_cl_sha"],
    ...
    if environment_profile != {
        ...
        "schema_version": "vllm-cl-environment-profile/v1",
    }:
        return False
...
def build_v2_long_prefix(run_spec: dict[str, Any], output_allowance: int) -> dict[str, Any]:
    ...
    if not isinstance(proof, dict) or set(proof) != expected or (
        proof["schema_version"] != "vllm-cl-long-prefix-proof/v1"
...
def run_v2(
    run_spec: dict[str, Any], vllm_cl_root: Path, before: dict[str, str]
) -> int:
    guards = run_spec["repository_guards"]
    if Path(guards[1]).resolve() != vllm_cl_root.resolve():
        return CONTRACT.invalid_input_report(
            ValueError("v2 guarded vllm-cl root mismatch"), vllm_cl_root, before
        )
    before_guards = {
        root: before if Path(root).resolve() == vllm_cl_root.resolve()
        else CONTRACT.repository_snapshot(Path(root))
        for root in guards
    }
    target_bindings = {
        guards[0]: run_spec["target"]["vllm_sha"],
        guards[1]: run_spec["target"]["vllm_cl_sha"],
    }
    if any(before_guards[root]["head"] != expected for root, expected in target_bindings.items()):
        return CONTRACT.invalid_input_report(
            ValueError("v2 target repository identity mismatch"), vllm_cl_root, before
        )
    ...
    if destination.exists():
        return CONTRACT.invalid_input_report(
            ValueError("v2 artifact destination must not exist"), vllm_cl_root, before
        )
    try:
    ...
    if digest(executable.read_bytes()) != launch["executable_digest"]:
        return CONTRACT.invalid_input_report(
            ValueError("launcher executable digest mismatch"), vllm_cl_root, before
        )
    ...
    if digest(smi_executable.read_bytes()) != run_spec["hardware_observation"]["executable_digest"]:
        return CONTRACT.invalid_input_report(
            ValueError("SMI executable digest mismatch"), vllm_cl_root, before
        )
    ...
    if tool_executable is not None and digest(Path(tool_executable).read_bytes()) != run_spec["hardware_observation"]["tool_digest"]:
        return CONTRACT.invalid_input_report(
            ValueError("SMI tool digest mismatch"), vllm_cl_root, before
        )
    for name in ("model", "tokenizer", "processor"):
        ...
        try:
            ...
            return CONTRACT.invalid_input_report(error, vllm_cl_root, before)
        if actual != expected:
            return CONTRACT.invalid_input_report(
                ValueError(f"{name} asset digest mismatch"), vllm_cl_root, before
            )
    ...
    reference = {"digest": result["digest"], "schema_version": "vllm-cl-result-reference/v2", "uri": result_path.as_uri()}
...
def main() -> int:
    ...
    try:
        before = (
            ...
            else CONTRACT.repository_snapshot(arguments.vllm_cl_root)
        )
        run_spec = json.loads(arguments.run_spec.read_text(encoding="utf-8"))
        contract_check = CONTRACT.validate_contract(run_spec, arguments.vllm_cl_root)
        if contract_check["status"] != "passed":
            raise ValueError(contract_check["code"])
        if run_spec["schema_version"] == "vllm-cl-run-spec/v2":
            if arguments.endpoint is not None:
                raise ValueError("--endpoint is not supported by run-spec/v2")
            return run_v2(run_spec, arguments.vllm_cl_root, before)
        ...
        return CONTRACT.invalid_input_report(
            error, arguments.vllm_cl_root, before
        )
    ...
    try:
        after = CONTRACT.repository_snapshot(arguments.vllm_cl_root)
