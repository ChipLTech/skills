...
- Existing vLLM-CL profiler scopes are aggregate and do not provide request-correlated rank generation closure.
...
Instrumentation changes must be owned and approved in the repository that owns each signal. Ticket 06 and skills-owned 06P must not patch vLLM, vllm-cl, PyTorch DLC Backend, DLCSynapse, DLC Runtime, DLC Custom Kernel, DLCCL, or `chipltech_smi_lib` to bypass this prerequisite.
