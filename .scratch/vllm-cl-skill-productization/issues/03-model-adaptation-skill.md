...
- Read-only vLLM and vllm-cl repository identities and worktree states.
...
- Read-only dry runs exercise the skill through user-style prompts and inspect its structured final status; they do not execute model code or edit vllm-cl.
...
- [ ] Ticket 01 and all dry-run fixtures pass without modifying vllm-cl.

## Stop Conditions

- Required model identity, configuration, tokenizer, processor, deployment profile, or contract is missing or ambiguous.
- The skill would need to edit vllm-cl to complete this ticket.
