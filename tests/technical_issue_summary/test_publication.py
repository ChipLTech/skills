...
IDENTITY = "technical-issue-summary"


class TechnicalIssueSummaryPublicationTests(unittest.TestCase):
    ...
    def test_summary_contract_preserves_evidence_boundaries(self):
        skill = (
        ...
        for required in (
            ...
            "Claim Boundary",
            "debugging-workflows/technical-issue-summary.md",
        ):
            self.assertIn(required, skill)

        diagnosis = (
            ROOT / "skills" / "engineering" / "diagnosing-bugs" / "SKILL.md"
        ).read_text(encoding="utf-8")
        router = (
            ROOT / "skills" / "engineering" / "chipltech-context" / "SKILL.md"
        ).read_text(encoding="utf-8")
        self.assertIn(f"/{IDENTITY}", diagnosis)
