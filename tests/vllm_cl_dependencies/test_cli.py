...
CLI = ROOT / "scripts" / "qualify-vllm-cl-dependencies.py"
VALIDATOR = ROOT / "scripts" / "validate-vllm-cl-contracts.py"
RUNNER = ROOT / "scripts" / "run-vllm-cl-smoke.py"
POLICY = ROOT / "config" / "vllm-cl" / "ticket06-dependency-exceptions.json"
MAIN_TO_MAIN_POLICY = (
    ROOT / "config" / "vllm-cl" / "ticket06-main-to-main-operational-policy.json"
)
EXPECTED_KEYS = {
...
class DependencyQualificationCliTests(unittest.TestCase):
    ...
    def run_cli(
        ...
        *,
        ...
        vllm_cl_root: Path = Path("/work/vllm-cl"),
        ...
        checks, exceptions, reasons = self.metadata_result(
            {
                ...
                "vllm-cl": ["opencv-python-headless<=4.11.0.86"],
            },
...
class DependencyQualificationCampaignTests(unittest.TestCase):
    ...
    def qualification(self) -> dict[str, object]:
        return {
            ...
            "imports": [{"name": "vllm_cl", "passed": True}],
            "metadata_checks": [
                {
            ...
            "native_modules": [
                ...
                {
                    "expected_root": "/work/vllm-cl",
                    "name": "vllm_cl.vllm_cl_C",
                    "origin": "/work/vllm-cl/vllm_cl/vllm_cl_C.so",
                    "passed": True,
                },
            ],
            "reasons": [],
            "requirements_path": "/work/skills/requirements-vllm-cl-contracts.txt",
            "schema_version": "vllm-cl-dependency-qualification/v1",
            "status": "passed",
        }
    ...
    def test_operational_campaign_requires_dependency_closure_entries(self) -> None:
        ...
        self.assertEqual(
            ...
            (ROOT / "requirements-vllm-cl-contracts.txt").resolve(),
        )
