...
class KnowledgeCliTests(unittest.TestCase):
    ...
    def copied_package(self):
        ...
        for relative in (
            ...
            "prompt-examples/vllm-cl-model-adaptation.md",
            "prompt-examples/vllm-cl-main-to-main-upgrade.md",
        ):
    ...
    def test_governed_documents_reuse_duplicate_quality_gate_with_role_context(self):
        ...
        for addition in additions:
            with self.subTest(addition=addition):
                ...
                prompt = root / "prompt-examples/vllm-cl-model-adaptation.md"
                ...
                self.assertEqual(check["path"], "prompt-examples/vllm-cl-model-adaptation.md")
    ...
    def test_required_and_local_links_must_resolve(self):
        ...
        readme.write_text(readme.read_text().replace("prompt-examples/vllm-cl-model-adaptation.md", "prompt-examples/missing.md"))
        ...
        root, manifest = self.copied_package()
        decision = root / "vllm-cl/model-adaptation-and-main-to-main-decisions.md"
    ...
    def test_prompt_frontmatter_is_exact_and_rejects_unsupported_claims(self):
        mutations = [
            ...
            (lambda text: text.replace("prompt_schema: vllm-cl-reusable-prompt/v1", "prompt_schema: v2"), "prompt.invalid"),
            (lambda text: text.replace("skill_identity: model-adaptation", "skill_identity: invented-owner"), "prompt.invalid"),
            (lambda text: text.replace("shared_contract: vllm-cl-contract/v1", "shared_contract: missing"), "prompt.invalid"),
    ...
    def test_prompt_body_cannot_claim_acceptance_or_finalization(self):
        ...
        for addition in additions:
            with self.subTest(addition=addition):
                root, manifest = self.copied_package()
                prompt = root / "prompt-examples/vllm-cl-main-to-main-upgrade.md"
                prompt.write_text(prompt.read_text() + "\n" + addition + "\n")
                result = self.run_cli("knowledge-package", manifest, root)
                self.assertEqual(result.returncode, 20, result.stdout)
                check = json.loads(result.stdout)["checks"][0]
