...
class ModelAdaptationCliTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.vllm_cl_root = Path(cls._repository.name)
        subprocess.run(["git", "init", "-q", "-b", "dev-skills", str(cls.vllm_cl_root)], check=True)
        (cls.vllm_cl_root / "identity.txt").write_text("fixture\n", encoding="utf-8")
        subprocess.run(["git", "-C", str(cls.vllm_cl_root), "add", "identity.txt"], check=True)
        subprocess.run([
            "git", "-C", str(cls.vllm_cl_root),
    ...
    def test_operational_consumer_rejects_synthetic_passed_json(self):
        document = {
            ...
            "schema_version": "vllm-cl-model-adaptation-operational/v1",
        }
    ...
    def attach_sealed_documents(
        ...
        *,
        ...
        guarded_head = subprocess.run(
            ["git", "-C", str(self.vllm_cl_root), "rev-parse", "HEAD"],
            check=True,
            capture_output=True,
            text=True,
        ).stdout.strip()
        ...
        result = {
            "schema_version": "vllm-cl-result-evidence/v1",
        ...
        if parent:
            handoff = {
                "schema_version": "vllm-cl-parent-child-handoff/v1",
    ...
    def test_sealed_run_is_bound_to_workflow_preflight_and_tp(self) -> None:
        mutations = [
            ...
            lambda bundle: bundle["run_spec"]["target"].update(vllm_cl_sha="9" * 40),
        ]
        for mutation in mutations:
            with self.subTest(mutation=repr(mutation)):
                def mutate(bundle, mutation=mutation):
