...
class ModelZooPublicationTests(unittest.TestCase):
    ...
    def test_skill_document_locks_public_cross_repository_contract(self):
        ...
        self.assertIn("requires exact-artifact revalidation", text)
        self.assertIn("every execution-relevant binary and adapter digest is unchanged", text)
        self.assertIn("evidence from an older candidate cannot be transferred", tyd)
        ...
        self.assertIn("Performance-diagnostic and formal benchmark profiles are separate epochs", runtime)
        self.assertIn("forced device synchronization", runtime)
        self.assertIn("cannot satisfy `benchmark_workload_pass`", runtime)
        self.assertIn("formal uninstrumented benchmark", runtime)
        self.assertIn("Delegate hotspot diagnosis to `diagnosing-bugs`", text)
