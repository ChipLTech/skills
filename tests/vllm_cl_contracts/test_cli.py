...
class ContractCliTests(unittest.TestCase):
    ...
    def run_v2_contract(self, mutate=lambda document: None):
        document = {
            ...
            "hardware_observation": {
                ...
                "adapter_version": "vllm-cl-smi-adapter/v1",
            ...
            "repository_guards": ["/work/vllm", "/work/vllm-cl"],
            "requests": [
            ...
            "schema_version": "vllm-cl-run-spec/v2",
            "target": {
    ...
    def configure_operational_v2(document, tensor_parallel_size=1):
        launcher = ROOT / "scripts" / "launch-vllm-cl-server.py"
        smi_adapter = ROOT / "scripts" / "observe-cltech-smi.py"
        qualification_adapter = (
            ROOT / "scripts" / "qualify-vllm-cl-smi-environment.py"
        )
        role = (
        ...
        document["launch"].update(
            ...
            arguments=[
                ...
                "--expected-vllm-cl-root", "/work/vllm-cl",
            ],
        )
    ...
    def test_valid_run_spec_is_deterministic(self) -> None:
        ...
        self.assertEqual(report["contract_version"], "vllm-cl-contract/v1")
    ...
    def test_ticket03_public_targets_are_registered(self) -> None:
        fixtures = ROOT / "tests" / "vllm_cl_model_adaptation" / "fixtures"
    ...
    def test_ticket04_public_targets_are_registered(self) -> None:
        fixtures = ROOT / "tests" / "vllm_cl_main_to_main" / "fixtures"
    ...
    def test_ticket05_public_targets_are_registered(self) -> None:
        fixtures = ROOT / "tests" / "vllm_cl_knowledge" / "fixtures"
    ...
    def test_run_spec_rejects_non_full_vllm_cl_identity(self) -> None:
        fixture = FIXTURES / "negative" / "invalid-vllm-cl-sha.json"
        result = self.run_cli("contract", str(fixture))
        self.assertEqual(result.returncode, 20)
        report = json.loads(result.stdout)
        self.assertEqual(report["checks"][0]["code"], "contract.missing_identity")
        self.assertEqual(report["checks"][0]["path"], "$.target.vllm_cl_sha")
    ...
    def test_invalid_repository_root_is_not_verified(self) -> None:
        ...
        result = subprocess.run(
            [
                ...
                "/work/chipltech-knowledge-base",
                "--vllm-cl-root",
                "/tmp/kilo/not-a-vllm-cl-repository",
                "contract",
                str(fixture),
            ],
            capture_output=True,
    ...
    def test_artifact_destination_cannot_enter_guarded_repository(self) -> None:
        result = self.run_mutated_contract(
            ...
            lambda document: document.update(
                artifact_destination="/work/vllm-cl/ticket-artifacts"
            ),
        )
        self.assertEqual(result.returncode, 20)
        report = json.loads(result.stdout)
    ...
    def test_package_rejects_structured_duplicate_quality_gates(self) -> None:
        ...
        for replacement in (
            "not_shared_contract: vllm-cl-contract/v1",
            "shared_contract: vllm-cl-contract/v1-extra",
        ):
            with self.subTest(replacement=replacement), tempfile.TemporaryDirectory() as directory:
                ...
                knowledge.write_text(
                    knowledge.read_text().replace(
                        "shared_contract: vllm-cl-contract/v1", replacement
                    )
                )
                result = self.run_cli("package", str(target_root / "target.json"))
                self.assertEqual(result.returncode, 40, result.stdout)
    ...
    def test_exact_markers_wrapped_commands_and_shell_fences_are_enforced(self) -> None:
        additions = [
            ...
            "$ conda run python3 -m vllm_cl.smoke_runner --run-spec spec.json",
            "$ /usr/bin/env python3 -m vllm_cl.smoke_runner --run-spec spec.json",
