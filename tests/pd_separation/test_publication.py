...
class PDSeparationPublicationTests(unittest.TestCase):
    def test_publication_surfaces_match_frontmatter(self):
        ...
        self.assertIn(f"/{IDENTITY} 使用单机 TCP", chinese_readme)
    ...
    def test_skill_locks_transport_and_claim_boundaries(self):
        ...
        for required in ("request-correlated", "single_node_tcp", "single_node_lyp_full", "cross_machine_tcp", "blocked_cleanup_incomplete", "Device execution never authorizes", "HTTP 200", "Cross-machine deployment defaults to TCP"):
