...
RUNNER = ROOT / "scripts" / "run-vllm-cl-smoke.py"
VALIDATOR = ROOT / "scripts" / "validate-vllm-cl-contracts.py"
SMI_OBSERVER = ROOT / "scripts" / "observe-cltech-smi.py"
SMI_PREFLIGHT = ROOT / "scripts" / "qualify-vllm-cl-smi-environment.py"
...
class SmokeRunnerCliTests(unittest.TestCase):
    def run_v2_fixture(
        ...
        with tempfile.TemporaryDirectory(dir="/tmp/kilo") as directory:
            ...
            (tokenizer / "fixture-tokenizer.json").write_text(json.dumps({
                "schema_version": "vllm-cl-fixture-whitespace-tokenizer/v1"
            }))
            model = root / "model"
            model.mkdir()
            ...
            spec = {
                ...
                "hardware_observation": {
                    "provider_class": "fixture", "adapter_version": "vllm-cl-smi-adapter/v1",
                ...
                "mode": "diagnostic_only", "repository_guards": ["/work/vllm", "/work/vllm-cl"],
                "requests": [
                    {
                ...
                "run_id": run_id, "schema_version": "vllm-cl-run-spec/v2",
                "target": {
                    "vllm_sha": subprocess.run(["git", "-C", "/work/vllm", "rev-parse", "HEAD"], check=True, capture_output=True, text=True).stdout.strip(),
                    "vllm_cl_sha": subprocess.run(["git", "-C", "/work/vllm-cl", "rev-parse", "HEAD"], check=True, capture_output=True, text=True).stdout.strip(),
            ...
            if tamper_tp_derivation:
                ...
                approval_path.write_text(json.dumps({
                    ...
                    "schema_version": "vllm-cl-model-adaptation-approval/v1",
                    "tensor_parallel_size": 1,
                }))
                derivation_path.write_text(json.dumps({
                    ...
                    "schema_version": "vllm-cl-tp-derivation/v1",
                    "target": spec["target"],
                }))
            ...
            result = subprocess.run(
                [sys.executable, str(RUNNER), "--run-spec", str(spec_path), "--vllm-cl-root", "/work/vllm-cl"],
    ...
    def test_v2_runner_owns_fixture_lifecycle_but_cannot_promote_it(self):
        with tempfile.TemporaryDirectory(dir="/tmp/kilo") as directory:
            ...
            spec = {
                ...
                "run_id": "ticket06-v2-fixture", "schema_version": "vllm-cl-run-spec/v2",
                "target": {
                    "vllm_sha": subprocess.run(["git", "-C", "/work/vllm", "rev-parse", "HEAD"], check=True, capture_output=True, text=True).stdout.strip(),
                    "vllm_cl_sha": subprocess.run(["git", "-C", "/work/vllm-cl", "rev-parse", "HEAD"], check=True, capture_output=True, text=True).stdout.strip(),
                    "manifest_digest": "sha256:" + "d" * 64,
            ...
            evidence = json.loads(Path(reference["uri"].removeprefix("file://")).read_text())
            self.assertEqual(result.returncode, 0, json.dumps(evidence["diagnostics"]))
            self.assertEqual(evidence["schema_version"], "vllm-cl-result-evidence/v2")
            ...
            campaign = {
                ...
                "schema_version": "vllm-cl-ticket06-operational-index/v1",
    ...
    def run_scenario(self, scenario, timeouts=None):
        ...
        spec = {
            ...
            "schema_version": "vllm-cl-run-spec/v1",
            "target": {"vllm_sha": "3" * 40, "vllm_cl_sha": "4" * 40, "manifest_digest": "sha256:" + "5" * 64},
        ...
        spec_validation = subprocess.run(
            [sys.executable, str(VALIDATOR), "--skills-root", str(ROOT), "--knowledge-root", "/work/chipltech-knowledge-base", "--vllm-cl-root", "/work/vllm-cl", "contract", str(spec_path)],
            capture_output=True, text=True,
        )
        self.assertEqual(spec_validation.returncode, 0, spec_validation.stderr)
        result = subprocess.run(
            [sys.executable, str(RUNNER), "--run-spec", str(spec_path), "--endpoint", f"http://127.0.0.1:{port}", "--vllm-cl-root", "/work/vllm-cl"],
            capture_output=True, text=True, timeout=15,
        )
        reference = json.loads(result.stdout)
        evidence = json.loads(Path(reference["uri"].removeprefix("file://")).read_text())
        validation = subprocess.run(
            [sys.executable, str(VALIDATOR), "--skills-root", str(ROOT), "--knowledge-root", "/work/chipltech-knowledge-base", "--vllm-cl-root", "/work/vllm-cl", "contract", reference["uri"].removeprefix("file://")],
            capture_output=True, text=True,
        )
        self.assertEqual(validation.returncode, 0, validation.stderr)
