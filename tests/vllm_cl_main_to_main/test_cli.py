...
def load_validator():
    spec = importlib.util.spec_from_file_location("vllm_cl_contract", CLI)
...
class MainToMainCliTests(unittest.TestCase):
    def validate_operational_policy_children(self, mutate_policy=None, mutate_specs=None):
        ...
        policy = {
            ...
            "schema_version": "vllm-cl-main-to-main-operational-policy/v1",
            "target": target,
        }
        if mutate_policy:
        ...
        document_path.write_text(json.dumps({
            ...
            "schema_version": "vllm-cl-main-to-main-operational/v1",
        }))
        ...
        with mock.patch.object(
            ...
            return validator.validate_main_to_main_operational(
                document_path, Path("/work/vllm-cl")
            )
    ...
    def setUpClass(cls):
        ...
        cls.main_root = Path(cls.synthetic_repository.name) / "vllm-cl"
        subprocess.run(
            ["git", "clone", "--quiet", "--no-hardlinks", "/work/vllm-cl", str(cls.main_root)],
            check=True,
        )
        subprocess.run(
    ...
    def attach_child(self, parent, assignment_index=0, status="not_verified", environment="fake_server", eligible=False):
        ...
        child = json.loads((ROOT / "tests/vllm_cl_model_adaptation/fixtures/contract-available-not-verified.json").read_text())
        ...
        run_spec = {
            "schema_version": "vllm-cl-run-spec/v1", "contract_kind": "run_spec",
            "run_id": assignment["child_run_id"], "workflow": "model_adaptation", "mode": "acceptance" if eligible else "diagnostic_only",
            "target": {"vllm_sha": parent["target"]["vllm_sha"], "vllm_cl_sha": parent["candidate_vllm_cl_sha"], "manifest_digest": parent["manifest_impact"]["manifest_digest"]},
        ...
        result = {
            "schema_version": "vllm-cl-result-evidence/v1", "contract_kind": "result_evidence",
        ...
        handoff = {
            "schema_version": "vllm-cl-parent-child-handoff/v1", "contract_kind": "parent_child_handoff",
            "parent_run_id": parent["parent_run_id"], "child_run_id": run_spec["run_id"],
            "target_vllm_sha": parent["target"]["vllm_sha"], "candidate_vllm_cl_sha": parent["candidate_vllm_cl_sha"],
    ...
    def test_candidate_revision_is_bound_to_guarded_head(self):
        result = self.run_bundle(
            lambda bundle: bundle.update(candidate_vllm_cl_sha="6" * 40)
        )
        self.assertEqual(result.returncode, 20)
        report = json.loads(result.stdout)
        self.assertEqual(report["checks"][0]["code"], "contract.identity_mismatch")
        self.assertEqual(report["checks"][0]["path"], "$.candidate_vllm_cl_sha")
    ...
    def test_branch_mismatch_blocks_before_analysis(self):
        ...
        result = self.run_bundle(mutate, guarded_root="/work/vllm-cl")
    ...
    def test_dirty_guarded_repository_derives_non_unique_revision_blocker(self):
        with tempfile.TemporaryDirectory(dir="/tmp/kilo") as directory:
            guarded_root = Path(directory) / "vllm-cl"
            subprocess.run(
                ["git", "clone", "--quiet", "--no-hardlinks", "/work/vllm-cl", str(guarded_root)],
                check=True,
            )
            subprocess.run(
    ...
    def test_parent_child_target_candidate_model_deployment_result_and_dependency_mismatches_reject(self):
        mutations = [
            ...
            lambda bundle, row: bundle.update(candidate_vllm_cl_sha="6" * 40),
